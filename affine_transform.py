...
import numpy as np
from pokemon_image import PokemonImage
...
def apply_affine_transform(pokemon_image, angle, scale, translate, shear):
    ...
    transformed_image = F.affine(
        pokemon_image.original_image, angle, translate, scale, shear, fill=None
    )
    ...
    if not check_annotations(transformed_annotations):
        ...
        return PokemonImage(transformed_image, transformed_annotations)
...
def check_annotations(arr):
    if np.any(arr < 0):
        return False
    if np.any(arr[:, 0] > 2048):
        return False
    if np.any(arr[:, 1] > 1536):
        return False
    return True
